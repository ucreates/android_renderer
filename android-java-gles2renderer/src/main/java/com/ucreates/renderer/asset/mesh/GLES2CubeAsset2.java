...
public class GLES2CubeAsset2 extends GLES2BaseAsset {
    public GLES2CubeAsset2(float width, float height, float depth, GLES2Color color) {
        this.width = width;
        this.height = height;
        this.depth = depth;
        this.color = color;
    ...
    public void create() {
        ...
        for (int i = 0; i < indiciesLength; i += GLES2Renderer.DIMENSION3D) {
            int idx1 = indicies[i];
            int idx2 = indicies[i + 1];
            int idx3 = indicies[i + 2];
            int v1idx = idx1 * GLES2Renderer.DIMENSION3D;
            int v2idx = idx2 * GLES2Renderer.DIMENSION3D;
            int v3idx = idx3 * GLES2Renderer.DIMENSION3D;
            ...
            surfaceNormals[idx1] = GLES2Normal.add(surfaceNormal1, triangleNormal);
            surfaceNormals[idx2] = GLES2Normal.add(surfaceNormal2, triangleNormal);
            surfaceNormals[idx3] = GLES2Normal.add(surfaceNormal3, triangleNormal);
        }
        int vnidx = 0;
        for (int i = 0; i < vertexCount; i++) {
            Float3 surfaceNormal = GLES2Normal.toOne(surfaceNormals[i]);
            vertexNormals[vnidx] = surfaceNormal.x;
            vertexNormals[vnidx + 1] = surfaceNormal.y;
            vertexNormals[vnidx + 2] = surfaceNormal.z;
            vnidx += GLES2Renderer.DIMENSION3D;
        }
