...
import com.ucreates.renderer.asset.MaterialAsset;
...
public class ObjAsset extends BaseAsset {
    private HashMap<String, Material> materials = null;
    public ArrayList<Mesh> subMeshes;
    public ObjAsset() {
        this.vertex = new VertexArray(GLES1Renderer.DIMENSION3D);
        this.subMeshes = new ArrayList<Mesh>();
    }
    @Override
    ...
    private void addSubMesh(Mesh mesh, ArrayList<Float> vertexArray, ArrayList<Float> colorArray, ArrayList<Float> normalArray, ArrayList<Float> uvArray, int vertexCount) {
        if (null == mesh) {
            return;
        }
        VertexArray vertex = new VertexArray(GLES1Renderer.DIMENSION3D);
        ...
        if (null != this.materials) {
            Material material = this.materials.get(mesh.useMaterial);
            if (null != material) {
