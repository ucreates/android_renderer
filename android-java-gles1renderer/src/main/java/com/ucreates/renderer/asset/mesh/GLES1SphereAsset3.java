...
public class GLES1SphereAsset3 extends GLES1BaseAsset {
    private int divideCount;
    private float radius;
    public GLES1SphereAsset3(float radius, int divideCount, GLES1Color color) {
        this.radius = radius;
        this.color = color;
    ...
    public void create() {
        ...
        int originVerticesMemsize = originVerticesLength * GLES1Allocator.GL_FLOAT_SIZE;
        int colorsMemsize = colorsLength * GLES1Allocator.GL_FLOAT_SIZE;
        int vertexNormalsMemsize = vertexNormalsLength * GLES1Allocator.GL_FLOAT_SIZE;
        int surfaceNormalsMemsize = surfaceNormalsLength * GLES1Allocator.GL_FLOAT_SIZE;
    ...
    public void create(String texturePath, Context context) {
        ...
        int originUVSLength = originVertexCount * GLES1Renderer.DIMENSION2D;
        int vertexNormalsLength = verticesLength;
        int surfaceNormalsLength = vertexCount;
        int verticesMemsize = verticesLength * GLES1Allocator.GL_FLOAT_SIZE;
        int originVerticesMemsize = originVerticesLength * GLES1Allocator.GL_FLOAT_SIZE;
        ...
        int originUVSMemsize = originUVSLength * GLES1Allocator.GL_FLOAT_SIZE;
        int vertexNormalsMemsize = vertexNormalsLength * GLES1Allocator.GL_FLOAT_SIZE;
        int surfaceNormalsMemsize = surfaceNormalsLength * GLES1Allocator.GL_FLOAT_SIZE;
        float[] verticies = new float[verticesMemsize];
        float[] originVerticies = new float[originVerticesMemsize];
        ...
        for (int i = 0; i < this.divideCount; i++) {
            ...
            for (int j = 0; j < this.divideCount; j++) {
                ...
                originVerticies[vidx + 1] = y * this.radius;
                originVerticies[vidx + 2] = z * this.radius;
                originUVs[uvidx] = (float) u * this.texture.uvRatio.x;
                originUVs[uvidx + 1] = (float) v * v1ratio + v2ratio;
                vidx += GLES1Renderer.DIMENSION3D;
        ...
        for (int j = 0; j < this.divideCount - 1; ++j) {
            ...
            for (int i = 0; i < this.divideCount; ++i) {
                ...
                int v3idx = v3 * GLES1Renderer.DIMENSION3D;
                int v4idx = v4 * GLES1Renderer.DIMENSION3D;
                int uv1idx = v1 * GLES1Renderer.DIMENSION2D;
                int uv2idx = v2 * GLES1Renderer.DIMENSION2D;
                int uv3idx = v3 * GLES1Renderer.DIMENSION2D;
                int uv4idx = v4 * GLES1Renderer.DIMENSION2D;
                if (originVerticesLength == v4idx) {
                    verticies[vidx] = originVerticies[v1idx];
                    verticies[vidx + 1] = originVerticies[v1idx + 1];
                    verticies[vidx + 2] = originVerticies[v1idx + 2];
                    ...
                    vidx += GLES1Renderer.DIMENSION3D * 3;
                    uviidx += GLES1Renderer.DIMENSION2D * 3;
                } else {
                    verticies[vidx] = originVerticies[v1idx];
                    verticies[vidx + 1] = originVerticies[v1idx + 1];
                    verticies[vidx + 2] = originVerticies[v1idx + 2];
                    ...
                    vidx += GLES1Renderer.DIMENSION3D * 4;
                    uviidx += GLES1Renderer.DIMENSION2D * 4;
                }
            }
        }
        int v1 = 0;
